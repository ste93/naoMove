...
def novelty(choreography, population, tools, toolbox):
    ...
    # print pop_resulting
    r = min(len(archive), GeneticAlgorithm.Constants.max_arch)
...
def fitness(movesList):
    ...
    if arch_len == 0 and evaluation > GeneticAlgorithm.Constants.fitness_threshold: #case 1
...
def calculate_fitness_and_novelty(choreography, population, tools, toolbox):
    ...
    archive = Archive.getArchive()["archive"]
    if len(archive) > 0 and fitness_value > GeneticAlgorithm.Constants.fitness_threshold and novelty_value > GeneticAlgorithm.Constants.novelty_threshold:
...
def mutation(movesList):
    for _ in range(random.randint(1, GeneticAlgorithm.Constants.max_number_of_mutations)):
        movesList[random.randint(0,len(movesList)-1)] = random.choice(GeneticAlgorithm.Constants.list_of_moves.keys())
    return (movesList,)
